...
class GridCategoryCell extends StatefulWidget {
  ...
  GridCategoryCell(context, this.categoryDetail, this.index, this.clength, this.updateui) ;
...
//  List<bool> _selectedCard = [] ;
  ...
  selectCard(int cindex) async {
    for(int i = 0; i < widget.clength ; i++){
      if(i == cindex){
        Constants.gridcellStatus[widget.index] = !Constants.gridcellStatus[widget.index];
    ...
    for(int i = 0; i < widget.clength ; i++){
        if(Constants.gridcellStatus[i] == true){
          ...
          print('Catg expPoints: ' + await SharedpreferencesHelper.getCatExpPoints());
        }
    }
  ...
  //  print('Check selected status ' + Constants.gridcellStatus[5].toString()) ;
    return Stack(
      children: <Widget>[
                        ...
                        children: <Widget>[
                          Container(
                            ...
                            child: Padding(
                              padding:
                              const EdgeInsets
                                  .fromLTRB(8.0, 10.0, 8.0, 8.0),
                              child: Image.network(
                                'http://highflyapps.com/quizrn/assets/category_icons/' + widget.categoryDetail.categoryImage,
                                color: Colors.white,
                                height: cheight*0.06,
                                width: cheight*0.06,
                              ),
        ...
        Constants.gridcellStatus[widget.index] ? Positioned(
          bottom: 20,
          right: 20,
